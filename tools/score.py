...
def main() -> int:
    ...
    print(f"score: [{rs[-1]['fixture']}] " + line(rs[-1]))
    if rt:
        r = rt[-1]
    ...
    if "--gate" in sys.argv:
        last = rs[-1]
        same = [r for r in rs[:-1] if r["fixture"] == last["fixture"]]
        if same:
            prev = same[-1]
            if last["score"] < prev["score"] or last["miss"] > prev["miss"]:
                print(f"GATE: the newest run on {last['fixture']} ({last['score']:g}, {last['miss']} miss) scores below "
                      f"the previous run on the same fixture ({prev['date']}: {prev['score']:g}, {prev['miss']} miss). "
