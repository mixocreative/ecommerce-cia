...
| **LINE Pay**（direct, or via a gateway｜直連或經由金流轉接） | the direct-vs-via decision first; docs from `developers-pay.line.me` (Online API v3 and v4); HMAC signer cross-checked against an independent implementation; request probe with `--check` for the five status codes; error translator for the `returnCode` table｜先做「直連 vs 經由金流轉接」的決定；參閱 `developers-pay.line.me` 官方文件（Online API v3 及 v4）；HMAC 簽章器與獨立實作交叉比對；附 `--check` 的請求探針可讀五種狀態碼；針對 `returnCode` 對照表的錯誤碼轉譯器 | live on LINE Pay's sandbox host: wrong credentials → `1104 Merchant not found`, exactly as documented; signer known-answer equals Python's stdlib HMAC; the PASS path awaits a sandbox Channel ID｜在 LINE Pay 測試環境（sandbox）主機實測：錯誤的 Channel ID/Secret → `1104 Merchant not found`，與官方文件完全一致；簽章器的已知答案測試（known-answer test）與 Python 標準庫的 HMAC 完全一致；成功路徑（PASS）待取得測試環境 Channel ID 後驗證 |
...
| `tools/linepay/probe_request.php` against LINE Pay's sandbox host｜對 LINE Pay 測試環境（sandbox）主機執行 `tools/linepay/probe_request.php` | fake credentials → `1104 Merchant not found`, the documented code; `sign.php` MAC equals an independent stdlib HMAC; production refused without a flag｜假憑證 → 回傳文件記載的 `1104 Merchant not found`；`sign.php` 產生的 MAC 與獨立的標準庫 HMAC 完全一致；未帶指定旗標時拒絕連線正式環境 |
