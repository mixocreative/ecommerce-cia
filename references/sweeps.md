...
5. **The self-consistent codec.** A round-trip test — encrypt then decrypt, sign then verify — proves only that the code agrees with itself. Against a vendor's protocol that is not the question; the question is whether it agrees with the *vendor*. The NewebPay tools passed a round-trip selftest for a day before anyone checked them against the manual's own worked example (NDNF-1.2.5 §4.1 prints key, IV, plaintext, `TradeInfo` and `TradeSha`); the ECPay `CheckMacValue` test reproduced the published example but its "case-insensitive sort matters" sibling was vacuous — every key in the example starts uppercase, so ordinal and case-insensitive sorting give the same answer and the test could not fail. **For every external protocol the code speaks, find the vendor's published example and a test that reproduces it byte-for-byte; then find the negative that can actually tell** (a pair of keys that sort differently, a wrong IV, a tampered digit) **and prove the test goes red when the vector is tampered.** A codec without a vendor vector is a hypothesis with a green badge.
...
6. **Walk every UI action as its own mini-matrix.** Buttons, toggles, segmented controls, form submits, uploads/downloads, copy controls, refresh/recheck, setup/install, checkout, refund, dismiss, retry, bulk and destructive actions all need the same loop: before-click scope and availability; immediate acknowledgement; disabled/progress/running state; success/failure/cancel/retry result; next action; refresh/recheck if completion is detected later; final state. If a good click and a broken click look the same, the action is not closed.
...
6. **Walk action loops, not just pages.** Grep click handlers, submit handlers, route actions, mutation calls, background-job starters, copy-to-clipboard calls, refresh/recheck controls, save/delete/dismiss/retry/bulk actions and install/setup launchers. For each, prove the full loop named in the method: visible precondition, immediate acknowledgement, running state, terminal result, next action and refresh/recheck if the truth lives outside the page. A handler that returns 200 with no visible result is a surface gap.
7. **Then, and only then, the cross-product** - bounded deliberately.
