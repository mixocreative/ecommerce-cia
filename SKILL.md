...
| Direct LINE Pay, Stripe/Adyen | — | not yet written; run setup mode from the posture above and the vendor's manual, and say the guide is missing |

Shared: `tools/explain_error.py <code>` translates NewebPay, ECPay, PAYUNi and TapPay error codes into cause + next action; `tools/newebpay/readiness.py` renders the card for either provider.
